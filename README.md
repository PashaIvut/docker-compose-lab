...
### Зачем в Ч.1 использовался `--link` и почему в Compose он не нужен?

`--link` в `docker run` создавал статическую связь между контейнерами. В Compose это не нужно, потому что:
- Compose автоматически создает сеть для всех сервисов
- Все сервисы в одном файле `docker-compose.yml` находятся в одной сети по умолчанию

### Чем отличается `image` от `build` в Compose?

- **`image`**: Использует готовый образ из реестра (Docker Hub, private registry). Пример: `image: redis`
- **`build`**: Собирает образ из Dockerfile локально. 

### Почему нельзя масштабировать сервис с публикацией фиксированного порта на одном хосте?

На одном хосте нельзя привязать один и тот же порт к нескольким контейнерам — возникнет конфликт портов. Способы обойти:
1. **Динамические порты**: `"0:5000"` — Docker выберет свободный порт для каждого экземпляра
2. **Разные хосты**: Запускать экземпляры на разных машинах

### Что делает `docker compose down -v` и чем отличается от `down` без `-v`?

- **`docker compose down`**: Останавливает и удаляет контейнеры, удаляет сети, созданные Compose
- **`docker compose down -v`**: Делает всё то же + удаляет именованные тома (volumes), созданные Compose. Тома в Docker Compose — это пространства для хранения данных, внешние по отношению к контейнерам. Они позволяют данным сохраняться даже после остановки или перезапуска контейнеров. 

Флаг `-v` нужен для полной очистки данных (например, данные Redis в volume будут удалены).


### Как в Compose обеспечивается сетевое имя сервиса для обращения из другого сервиса?

Compose автоматически:
1. Создает сеть для проекта (по умолчанию)
2. Регистрирует имя сервиса как DNS-имя в этой сети
3. Позволяет обращаться к сервису по его имени из `docker-compose.yml`

Например, сервис `redis` доступен по имени `redis` (или `redis:6379` с портом) из любого другого сервиса в том же файле.
